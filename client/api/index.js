...
// Initialize Resend (safe: don't throw when API key missing in dev)
let resend;
try {
  if (process.env.RESEND_API_KEY) {
    resend = new Resend(process.env.RESEND_API_KEY);
  } else {
    console.warn('Resend API key missing — email sending disabled in this environment');
    resend = {
      emails: {
        send: async (opts) => {
          console.log('Resend mock send called with:', opts);
  ...
  resend = {
    emails: {
      send: async (opts) => {
        console.log('Resend mock (on error) send called with:', opts);
        return { id: `demo-email-${Date.now()}` };
      },
    },
  };
...
const addComment = async (req, res) => {
  try {
    ...
    // Debug: incoming addComment call
    console.log('client addComment called:', {
      author, isAnonymous: Boolean(isAnonymous), rating, type, userFingerprint, dbPresent: Boolean(db)
    });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Contact form route — save submissions to Firestore (client SDK)
    if (url === '/api/contact') {
      if (method !== 'POST') {
      ...
      if (db) {
        try {
          ...
          return res.status(201).json({ success: true, id: docRef.id, message: 'Contact saved (client SDK)'});
        } catch (err) {
          console.error('Firebase client contact save error:', err.message || err);
        }
      }

      const demoId = 'demo-' + Date.now();
      return res.status(201).json({ success: true, id: demoId, message: 'Contact received (demo)'});
    }
